...
public void deleteTxn(long txn_Id) throws JsonProcessingException {
    ...
    public TransactionFilterResponse getTxnSummaryWithFilterBased(String bank, String transactionFilter, LocalDate fromDate, LocalDate toDate) {
        try {
            ...
            transactionFilterResponse.setAllfilterTransactions(transactionList);
    ...
    public List<Transaction> getTransactionsByUserID(Long userId) {
        try {
        	logger.info("TransactionServiceImpl-getTransactionsByUserID() call started");
            List<Transaction> userTransactions = transactionRepo.findByUserId(userId);
            if (!userTransactions.isEmpty()) {
                logger.info("TransactionServiceImpl-getTransactionsByUserID() - User transactions retrieved: {}", userTransactions.size());
            	return userTransactions;
                }
            logger.error("TransactionServiceImpl-getTransactionsByUserID() - User with ID {} not found", userId);
            throw new CustomExceptions(HttpStatus.BAD_REQUEST.value(),"BAD_REQUEST","User with ID " + userId + " not found.","");
        } catch (Exception e) {
        	  logger.error("TransactionServiceImpl-getTransactionsByUserID() - Error occurred: {}", e.getMessage());
        	    throw e;    
